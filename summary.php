...
$user_id = $user["id"];
$token = $post["token"];
$all_transaction = mysqli_query($conn , "select * from transaction where deleted=0 and approval='' and done_by in (select id from user where room = ( select room from user where token= '".$token."'))");
